...
这个工作区用于本地学习和调试金融类 LLM 应用与 Agent 示例。项目包含 LangGraph、Agno 等多种 Agent 框架，也包含 Streamlit、RAG、MCP 和直接调用 DeepSeek 的应用。示例集中放在 `finance-llm-agent-demos` 目录，Agno 渐进式学习规划放在 `agno-demos`，Agno 开源 AgentUI 放在 `tools/agent-ui`，运行脚本放在 `scripts/` 和 `finance-llm-agent-demos/scripts/`。
...
各示例也支持在自己的项目目录下放 `.env`。加载优先级通常是：项目目录 `.env`、`finance-llm-agent-demos/.env`、工作区根目录 `.env`。
...
启动 13 知识图谱 RAG 与可验证引用：

```bash
cd finance-llm-agent-demos
./scripts/run_13_agent.sh
```
