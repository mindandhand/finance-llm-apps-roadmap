...
1. **环境配置**
   ...
   程序使用 `Path` 定位当前项目，并依次尝试加载项目目录、`finance-llm-agent-demos` 根目录和工作区根目录的 `.env`。`call_deepseek()` 从环境变量读取 `DEEPSEEK_API_KEY`、`DEEPSEEK_BASE_URL` 和 `DEEPSEEK_MODEL_ID`，通过 `requests` 直接调用 DeepSeek 的 `/chat/completions` 接口。
